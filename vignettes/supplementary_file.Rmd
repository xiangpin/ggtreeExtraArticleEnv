...
link-citations: yes
vignette: >
  %\VignetteIndexEntry{ ggtreeExtra: Compact visualization of richly annotated phylogenetic data}
...
**Shuangbin Xu, Zehan Dai, Pingfan Guo, Xiaocong Fu, Shanshan Liu, Lang Zhou, Wenli Tang, Tingze Feng, Meijun Chen, Li Zhan, Tianzhi Wu, Erqiang Hu and Guangchuang Yu\* **
...
overviewfig <- system.file("extdata", "overview.png", package="ggtreeExtraArticleEnv")
knitr::include_graphics(overviewfig, dpi=300)
```
...
x <- "ggtreeExtra\tR,package\tcircular,inward circular,rectangular,slanted,ellipse,round rectangular,radial\tHeatmap,scatter plot,simple bar chart,patter bar chart,stacked bar chart,dodged bar chart,boxplot,pattern boxplot,violin plot,dot intervals plot,density plot,line plot,pie chart,image plot,MSA[note],inset plot\tYes\tadd,modify,delete\tYes\tprogramming,and command,line\n
...
If you want to draw the examples of the supplemental file, you also need to install the following packages. You can refer the following command to install them.

```{r, eval=FALSE}

# For Fig.S3
# ggtree has been released to the Bioconductor, But the development version (> 2.5.1) was needed in here.
# You can install ggtree from the github repository by 'remotes', or you can install it by 
# 'BiocManager::install("ggtree")' if the Bioconductor (>=3.13) is released.
...
# Because ggpattern have not been released on 'CRAN', we
# can install it from the github repository.
...
# treeio has also been released to the Bioconductor, But the development version (>= 1.15.6) was needed 
# in here. You can install it from the github repository by 'remotes', or you can install it by 
# 'BiocManager::install(treeio)' if the Bioconductor (>=3.13) is released.
remotes::install_github("YuLab-SMU/treeio") 

install.packages(c("tidytree", "ggpmisc", "ggimage"))
...
The source codes and datasets to produce the figures can be obtained from the repository^[<https://github.com/YuLab-SMU/plotting-tree-with-data-using-ggtreeExtra>].

\newpage

## Docker image

To reproduce the results of the file easily, we also pushed the source code and datasets to the docker hub^[<https://github.com/xiangpin/ggtreeExtraArticleEnv>], which provides the environment to produce the figures. Users do not need to install the dependency packages.
...
## subplot B
tr <- read.tree(system.file("extdata","VertebrateGutMicrobiomes/annotated_host_tree.tre", 
                            package="ggtreeExtraArticleEnv"))
corda <- read.csv(system.file("extdata","VertebrateGutMicrobiomes/mantel.jaccard.pearson.csv", 
                              package="ggtreeExtraArticleEnv"))
corda$r <- abs(corda$r)
barda <- read.csv(system.file("extdata","VertebrateGutMicrobiomes/data_diet_bar.csv",
                              package="ggtreeExtraArticleEnv"), check.names=F)
...
cladeda <- read.csv(system.file("extdata","VertebrateGutMicrobiomes/data_clade_class.csv", 
                                package="ggtreeExtraArticleEnv"), check.names=F)
...
flightda <- read.csv(system.file("extdata","VertebrateGutMicrobiomes/data_flight_bar.csv", 
                                 package="ggtreeExtraArticleEnv"))
phylopicda <- read.csv(system.file("extdata","VertebrateGutMicrobiomes/data_phylopic_uid.csv", 
                                   package="ggtreeExtraArticleEnv"))
phylopicda$class <- factor(phylopicda$class, levels=c("Amphibia","Chelonia","Lepidosauria",
                                                      "Aves","Mammalia"))

fig <- ggtree(tr, layout="fan", open.angle=5, continuous=TRUE)
...
fig2 <- fig1 +
      ...
      scale_colour_manual(
          name="Host Class",
          values=c("#b2df8a","#33a02c","#fb9a99","#e31a1c","#EACB47","#6a3d9a"),
          guide=guide_legend(
                    keywidth=0.3,
                    keyheight=0.3,
                    order=2,
...
fig4 <- fig3 +
      ...
      scale_colour_manual(
          values=c("#b2df8a","#33a02c","#fb9a99","#EACB47","#6a3d9a"),
          guide="none"
      ) +
      theme(
...
library(ggnewscale)
library(tidytree)
tr <- read.raxml(system.file("extdata","Methanotroph/Methanotroph_rpS3_Modified_Alignment_RAxML", 
                             package="ggtreeExtraArticleEnv"))
# Optional, Root the tree to the archaea sequences
treeda <- root(tr, node=1402, edgelabel=TRUE)
root <- rootnode(treeda)
# read associated data
df <- read.csv(system.file("extdata", "Methanotroph/metadata.csv", package="ggtreeExtraArticleEnv"))
...
library(ggnewscale)
library(patchwork)
tree <- read.tree(system.file("extdata","HMP_tree/hmptree.nwk", package="ggtreeExtraArticleEnv"))
# the abundance and types of microbes
dat1 <- read.csv(system.file("extdata","HMP_tree/tippoint_attr.csv", package="ggtreeExtraArticleEnv"))
# the abundance of microbes at different body sites.
dat2 <- read.csv(system.file("extdata","HMP_tree/ringheatmap_attr.csv", package="ggtreeExtraArticleEnv"))
# the abundance of microbes at the body sites of greatest prevalence.
dat3 <- read.csv(system.file("extdata","HMP_tree/barplot_attr.csv", package="ggtreeExtraArticleEnv"))
...
tree <- read.tree(system.file("extdata","kegg/kegg.nwk", package="ggtreeExtraArticleEnv"))
# The attributes of tip point
dt1 <- read.csv(system.file("extdata","kegg/tippoint_attr.csv", package="ggtreeExtraArticleEnv"))
# The attributes of first ring
dt2 <- read.csv(system.file("extdata","kegg/firstring_attr.csv", package="ggtreeExtraArticleEnv"))
# The attributes of second ring
dt3 <- read.csv(system.file("extdata","kegg/secondring_attr.csv", package="ggtreeExtraArticleEnv"))
# The attrihutes of bar plot
dt4 <- read.csv(system.file("extdata","kegg/barplot_attr.csv", package="ggtreeExtraArticleEnv"))
...
alltax <- read.csv(system.file("extdata","Arabidopsis_leaf_microbiome/all_stain_taxonomy.csv", 
                               package="ggtreeExtraArticleEnv"))
linktab <- read.csv(system.file("extdata","Arabidopsis_leaf_microbiome/Interaction_link_tab.csv", 
                                package="ggtreeExtraArticleEnv"))
weighttab <- read.csv(system.file("extdata","Arabidopsis_leaf_microbiome/Interaction_weight.csv", 
                                  package="ggtreeExtraArticleEnv"))
tippoint <- read.csv(system.file("extdata","Arabidopsis_leaf_microbiome/stain_tippoint.csv", 
                                 package="ggtreeExtraArticleEnv"))
BGCsda <- read.csv(system.file("extdata","Arabidopsis_leaf_microbiome/BGCs_heatmap.csv", 
                               package="ggtreeExtraArticleEnv"))
...
p5 <- p4 +
      ...
      geom_fruit(
          data=weighttab,
          geom=geom_bar,
          mapping=aes(x=value, y=Strain, fill=Number),
          stat="identity",
          orientation="y",
          offset=0.48,
          pwidth=2,
