...
# Because ggpattern has not been released on 'CRAN', we
# need to install it from the github repository.
...
require("reshape2")
barda <- melt(barda, id.vars="ID", variable.name="Diet", value.name="mete")
...
require("reshape2")
intersectNums <- dcast(intersectNums, Isolation + Taxa ~ Number, sum)
intersectNums <- melt(intersectNums, id=c("Isolation", "Taxa"), variable.name="Number")
